    ...
    if(!pAArent) {
        ...
        context.storekeeper = (function() {
            ...
            function notify(_path, $, isNotifyParent, isSupressNotify) {
                ...
                //if(isNotifyParent) {
                //    if(arguments.length < 5) { // needed because memonic can be undefined
                //        if($.isValue) {
                //            const _parent$ = grab($._parent);
                //            notifyMemonic = _parent$.obj[$.seg];
                //        } else {
                //            notifyMemonic = $.obj;
                //        }
                //    }
                ...
                arr.forEach(l => { //console.log('notify', i, _
                    //if(updateListenerMemonic(l, notifyMemonic))
            ...
            //function updateListenerMemonic(cb, notifyMemonic) {
            //    // prevent two consequitive notifications of same value (even when value is undefined).
            //    if(cb.notifyMemonic === notifyMemonic) {
            //        // optmzed handling of 1st invocation which is always undefined
            //        if(notifyMemonic === undefined) {
            //            if(cb.hasOwnProperty('notifyMemonic')) {
            //                console.log('do NOT notify with undefined notifyMemonic', notifyMemonic);
            //                return;
            //            }
            //        } else {
            //            console.log('do NOT notify with', notifyMemonic);
            //            return;
            //        };
            //    }
            //    console.log('should notify with', cb.notifyMemonic, '->', notifyMemonic);
            //    cb.notifyMemonic = notifyMemonic;
            //    return true;
            //}
                ...
                if(params) { //console.log('   return params');
                    ...
                    for(let p in params) {
                        if(Object.prototype.hasOwnProperty.call(params, p)) {
                            ...
                            param = params[p]; //console.log('      param=',param, param._path);
            ...
            function on(path, cb, isSilent, isForce) { //console.log('asd', path);
                ...
                if(!isSilent) {
                    if($.obj || $.isValue || isForce) { //console.log({isForce});
                        ...
                        //    notifyMemonic = $.obj;
                        //}
                        //if(updateListenerMemonic(cb, notifyMemonic))
                        cb(_path, get);
                    }
            ...
            f.classList = function(name_path, options={}, cb) { //console.log({name_path, options});
                ...
                if(name_path) {
                    context.expressionfy(f, name_path, null, v => { // expressionlistener
                        //if(name_path.startsWith('$')) console.log({name_path, v}, f.elementName, when.is(v), when.transform(v));
