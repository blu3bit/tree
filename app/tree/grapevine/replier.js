 ...
 *   best:           quality = total own scores of these cards / amount of dowloaded cards
 *   most:           amount of downloaded content from peer / total downloaded cards (my) score from all peers
 *   peer score:     (best + most) / 2
 *   example:
 *           A   20 score / 120 total score downloaded = 0.16
 *               80 cards / 90 total                   = 0.88 most
...
export function createReplier({ß, log, peerProxy}) {                      //log?.n(0, 'createReplier');
    let weight = null,
        sumScore = 0,
        topup = 0,
        earned = 0,
    ...
    function addCardScore(score, oldScore) { //console.log({score, oldScore, sumScore});
    ...
    function createReply({nonce}) {
        const send = (data, cb) => {
            if(isResetGrace) {
                grace = ß.config.UPLOAD_QUOTA_DEFAULT_GRACE;
                isResetGrace = false;
            }
            peerProxy.createReplyAndSendMessage({...data, quota: getQuota()}, nonce, err => {
                if(err) {
